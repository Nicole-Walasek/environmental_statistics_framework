...
framework.applyModel <- function(data,
  ...
  df$m_apprEntropy <-
    map_dbl(data_models$data, function(data) {
      sample_entropy(data$y, r =rP*(sd(data$y)))
    })
  ...
  df$stat_apprEntropy <-
    map_dbl(data_models$data_stationary, function(data) {
      sample_entropy(data, r =rP*(sd(data)))
    })
  ...
  df$v_apprEntropy <-
    map_dbl(data_models$data, function(data) {
      sample_entropy(data$yVar, r =rP*(sd(data$yVar)))
    })
  ...
  df$r_apprEntropy <-
    map_dbl(data_models$data, function(data) {
      sample_entropy(data$yRes, r =rP*(sd(data$yRes)))
    })
  ...
  function(dataAll,
    ...
    if (type == "mean") {
      if('y' %in% names(results)){
        ...
        names(results)[names(results) == 'y_s'] <- 'originalData'
      }
      yLab <- ylab
...
framework.plotPublicationTest <-
  function(dataAll,
    ...
    if (typeCP == 'individual'){
      ...
      # and now for the variance
      if (typePenaltyVar == 'MBIC'){
        cpSegVar <-
          unlist(pmap(
            list(dataAll$data, dataAll$changePoints_MBIC, "ind", "Var"),
            changePoints_SegVals
    ...
    results$ppID <- droplevels(as.factor(results$ppID))
    ...
    if(regionFlag){
      ...
      region_names <- setNames(as.character(unique(data$region)),as.character(unique(data$ppID)))
      
      
      pNew_Sep <- pModel + facet_rep_wrap(~ factor(ppID, levels=sampleList), ncol = 1, scales = 'free_y', strip.position="right",labeller=as_labeller(region_names)) + theme(
      ...
      pNew_Sep <- pModel + facet_rep_wrap(~ factor(ppID, levels=sampleList), ncol = 1, scales = 'free_y',strip.position="right") + theme(
        strip.background = element_blank(),strip.text = element_textbox_highlight(size = 15, face = "bold")) +scale_y_continuous(limits= c(-3,NA),breaks = myBreaks, expand = c(0.1,0.05))
      
    }
    

    # add annotations autocorrelation
    labelVec = c()
    ...
    yPosDf <- results %>% group_by(ppID) %>% dplyr::summarise(min_y = min(y))
