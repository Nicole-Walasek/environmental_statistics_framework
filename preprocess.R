...
preprocess.adjust_trend <- function(dataTS, type){
  
  data_TS_x <-
    data.frame((sapply(dataTS["x", ], c)))
  data_TS_trend <-
    data.frame((sapply(dataTS["trend", ], c)))
...
preprocess.splitData <- function(data, splitValues){
  ...
  dataGrouped <- data %>% group_by(split)
  dataSplitted <- group_split(dataGrouped)
  
  for(idx in 1:length(splitValues)){
    splitVal <- splitValues[idx]
    dataSplitted[[idx+1]]$time <- dataSplitted[[idx+1]]$time - (splitVal) 
